...
public class QuickUnion_Weighted_FindMax {
  private int[] id;
  private int[] sizes;
  private int[] max;

  public QuickUnion_Weighted_FindMax(int N) {
...
//    System.out.printf("[root] id: %s\n", Arrays.toString(id));
  ...
  public void union(int p, int q) {
    ...
    if (sizes[r_p] >= sizes[r_q]) {
      id[r_q] = r_p;
      sizes[r_p] += sizes[r_q];
      sizes[r_q] = 0; // OPTIONAL
      if (max[r_p] >= max[r_q]) max[r_p] = max[r_q];
    } else {
      id[r_p] = r_q;
      sizes[r_q] += sizes[r_p];
      sizes[r_p] = 0; // OPTIONAL
      if (max[r_q] >= max[r_p]) max[r_q] = max[r_p];
    }
  ...
  public static void main(String[] args) {
    QuickUnion_Weighted_FindMax map=new QuickUnion_Weighted_FindMax(12);
    int i = 1, j = 2;
